...
Do not store secrets in the vault; keep only references to external secret locations when needed. When working inside a vault that defines an LLM Wiki schema, treat raw/source folders as read-mostly, write persistent synthesis into `wiki/`, and keep `wiki/index.md` and `wiki/log.md` current.
